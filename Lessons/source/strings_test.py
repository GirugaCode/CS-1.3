...
class StringsTest(unittest.TestCase):
    ...
    def test_contains_with_matching_patterns(self):
        ...
        assert contains('yeet', 'y') is True  
        assert contains('yeeeth', 'eth') is True
        assert contains('ryan', 'yan') is True
        assert contains('spongebob', 'bob') is True
        assert contains('bowl', 'owl') is True
        assert contains('mylegisasleep', 'legisa') is True 
    ...
    def test_contains_with_non_matching_patterns(self):
        ...
        assert contains('yeet', 'lala') is False  # partial pattern in text
        assert contains('yute', 'baba') is False  # first 2 letters, but not last
    ...
    def test_find_index_with_non_matching_patterns(self):
        ...
        assert find_index('yeet', 'ryan') is None
        assert find_index('yute', 'danh') is None
    ...
    def test_find_index_with_complex_patterns(self):
        ...
        # TODO: Write more test cases that check complex patterns or edge cases
        # You'll need a lot more than this to test your algorithm's robustness
        assert find_index('socket', 'ket') == 3
        assert find_index('computer', 'er') == 6
        assert find_index('paper', 'pa') == 0
    ...
    def test_find_all_indexes_with_matching_patterns(self):
        ...
        assert find_all_indexes('socket', 'ket') == [3]
        assert find_all_indexes('computer', 'er') == [6]
        assert find_all_indexes('paper', 'pa') == [0]
